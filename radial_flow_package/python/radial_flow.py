...
class WaveParams:
    ...
    @property
    def use_subleading_term(self):
        return self._cpp_obj.use_subleading_term
...
class RadialFlowSolver:
    ...
    def solve(self, r_max: float = 200.0, epsilon: float = 1e-6,
              ...
              n_periods: int = 2,
              max_r_factor: float = 5.0) -> dict:
        """
        从视界积分到 r_max（调用 C++ 核心，带收敛判据）

        参数：
            r_max: 最小积分终点（单位：r0）
            epsilon: 视界附近的起始偏移（相对于 r0）
            n_points: 输出点数
            convergence_threshold: 收敛阈值（震荡幅度相对于|A/B|的比例，默认0.01即1%）
            n_periods: 检测周期数（默认2个周期）
            max_r_factor: r 的最大扩展倍数（相对于 r_max，默认5倍）
        ...
        cpp_result = self._cpp_solver.solve(r_max, epsilon, n_points,
                                            convergence_threshold, n_periods, max_r_factor)
        ...
        # 修正：σ 实际上是 -A/B * e^{2iω r_*}
        # 需要对每个点乘以 e^{-2iω r_*} 来得到真正的 -A/B
        ...
        # 修正后的反射系数（取最后一个点）
        reflection_coeff = -sigma_corrected[-1]

        return {
            'r': r,
            'sigma': sigma_corrected,  # 返回修正后的 sigma
            'reflection_coeff': reflection_coeff,
            'A_over_B': reflection_coeff,
            'success': cpp_result.success
        }

    def compute_reflection_coefficient(self, r_max: float = 200.0) -> complex:
        """
        计算反射系数 A/B（包含龟坐标相位修正）

        σ 实际上是 -A/B * e^{2iω r_*}，需要乘以 e^{-2iω r_*} 修正
        """
        result = self.solve(r_max)
